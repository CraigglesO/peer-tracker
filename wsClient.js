...
var   WebSocket       = require('ws'),
      ws              = new WebSocket('ws://0.0.0.0:1337');
...
function Client (trackerHost, port, infoHash) {
  ...
  //setup server
  self.server = new WebSocket('ws://' + self.HOST + ':' + self.PORT);
  self.server.on('open', function open() {
    self.emit('listening', self.HOST);
    self.scrape();
  });
...
Client.prototype.message = function (msg, flags) {
  const self = this;
