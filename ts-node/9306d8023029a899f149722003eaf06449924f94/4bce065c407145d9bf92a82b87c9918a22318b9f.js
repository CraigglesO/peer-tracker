...
function handleMessage(msg, peerAddress, port, cb) {
    ...
    switch (action) {
        ...
        case ACTION_SCRAPE:
            ...
            // addresses return: 0 - leechers; 1 - seeders; 2 - hexedUp address-port pairs; 3 - resulting buffersize
            // Create a responce buffer:
            let responces = new buffer_1.Buffer((buf.length - 16));
            responces.fill(0);
            ...
            for (let i = 16; i < (buf.length - 16); i += 20) {
                ...
                client.mget([hash + ":seeders", hash + ":leechers", hash + ":completed"], (err, rply) => {
                    ...
                    let responce;
                    responce.writeUInt32BE(addresses[1], 8 + (i - 16)); // 8 + 12 * n  32-bit integer  seeders
                    responce.writeUInt32BE(rply[2], 12 + (i - 16)); // 12 + 12 * n 32-bit integer  completed
                    responce.writeUInt32BE(addresses[0], 16 + (i - 16)); // 16 + 12 * n 32-bit integer  leechers
                });
            }
            cb(responces);
            break;
...
//# sourceMappingURL=/Users/connor/Desktop/2017/PeerTracker/node/ts-node/9306d8023029a899f149722003eaf06449924f94/4bce065c407145d9bf92a82b87c9918a22318b9f.js.map