...
class Client extends events_1.EventEmitter {
    ...
    scrape() {
        ...
        if (!self.TRANSACTION_ID) {
            ...
            let buf = new buffer_1.Buffer(16 + self.HASH.length);
            buf.fill(0);
            buf.writeUInt32BE(connectionIdHigh, 0); // 0             64-bit integer  connection_id   0x41727101980
            buf.writeUInt32BE(connectionIdLow, 4); // 0             64-bit integer  connection_id   0x41727101980
            buf.writeUInt32BE(ACTION_SCRAPE, 8); // 8             32-bit integer  action          2 // scrape
            buf.writeUInt32BE(self.TRANSACTION_ID, 12); // 12            32-bit integer  transaction_id
            buf.write(self.HASH, 16, self.HASH.length, "hex"); // 16 + 20 * n   20-byte string  info_hash
            // Send Packet
            self.sendPacket(buf);
        }
    }
...
//# sourceMappingURL=/Users/connor/Desktop/2017/PeerTracker/node/ts-node/9306d8023029a899f149722003eaf06449924f94/f25f75eff7278005521683bf8b1753531cdb2b01.js.map